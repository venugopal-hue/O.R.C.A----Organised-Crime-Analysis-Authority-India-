...
export const Sidebar: React.FC = () => {
  ...
  const hasAccess = (itemId: string) => {
    ...
    // Default fallback to standard Investigation Dashboard
    const role = officerProfile.role || "Investigation Dashboard";
    
    if (itemId === "dashboard" || itemId === "settings") {
      return true; // Everyone has basic view of home and profile settings
    }

    if (role === "Investigation Dashboard") {
      // Standard Field Officers see only core operational tools and NO admin panel link
    ...
    if (role === "Administrative Dashboard - Level 1") {
      // Verification officers see only Verification tool and L1 Admin tabs
      const allowed = [
        "dashboard", "verification-document", "settings",
        "admin-dashboard", 
        "admin-applications", "admin-verification", "admin-directory"
      ];
      return allowed.includes(itemId);
    }
    
    if (role === "Administrative Dashboard - Level 2") {
      // Super admins see everything
      return true;
    }
    
    if (role === "IT Administration Dashboard") {
      // IT admins see only system infrastructure tools and NO investigation tools
      const allowed = [
        "dashboard", "settings",
        "admin-dashboard", 
                ...
                {accessExpanded && (
                  <div style={{ display: "flex", flexDirection: "column", marginTop: 4 }}>
                    {allowedAdminSidebarItems.filter(item => 
                      ["admin-dashboard", "admin-applications", "admin-directory", "admin-roles", "admin-verification"].includes(item.id)
                    ).map(item => {
